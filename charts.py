...
def generate_colored_bar_chart(zone_data, output_path="zone_chart.png"):
    labels = [x[0] for x in zone_data]
    values = [x[1] for x in zone_data]
    colors = ['red' if val >= 1 else 'green' for val in values]

    plt.figure(figsize=(10, 5))
    bars = plt.bar(labels, values, color=colors)
    plt.xticks(rotation=45, ha='right')
    plt.ylabel("Rainfall (mm)")
    plt.title("Zone-wise Rainfall")
    for bar, value in zip(bars, values):
